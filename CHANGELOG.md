...
- MEP agent stage A3 — room context menu «Тип помещения». Right-click over a room zone on the 2D floor plan selects it and opens a menu at the cursor with a СП 54 category submenu (`SPACE_CATEGORIES` radio items showing the normative rate per category); picking one patches `spaceCategory` via `updateNode` — one tracked undo step — and works for auto-detected rooms and manual zones alike. Shared label/rate dictionaries moved next to the schema enum (`SPACE_CATEGORY_LABELS` / `SPACE_CATEGORY_RATES` in `packages/core/src/schema/nodes/zone.ts`, re-exported through norms-types; the zone quantities panel now consumes them). Right-click plumbing rides a new editor-local emitter (`packages/editor/src/lib/floorplan-events.ts`) emitted from the floorplan registry layer for zone entries. W10 regression test added: reclassifying an auto zone then reshaping its walls regenerates geometry only — the update patch touches `polygon` alone and never resets user-set fields. Verified in the live editor: walls → auto-zone → right-click → «Кухня (газ)» persists across menu reopen, Ctrl+Z reverts. Total suite: **3257 pass / 1 skip / 0 fail**.
