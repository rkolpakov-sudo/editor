...
рекомендации оборудования; баланс П/В; митигации W1–W13) — **Этапы A1–A2 выполнены**:
...
`packages/nodes/src/duct-sketch/sketch-draft.ts`). Следующие — **Этап A3** (контекстное меню
помещения → «Тип помещения» по СП 54 + тест сохранения `spaceCategory` при регенерации зон,
W10), затем Этап B (ядро агента `packages/core/src/mep/agent/*`).
