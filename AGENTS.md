...
- Единицы — **полный переход на мм + ГОСТ-ряд** (Этап 1 выполнен: схемы duct хранят мм, schemaVersion 2, старые дюймы конвертируются на чтении через `migrateDuctUnitsToMm`).
...
Статус: **Этапы 0–12 выполнены** (справочник + план в `docs/mep/`; миграция единиц дюймы→мм + ГОСТ-ряд,
...
constants, norms-types, aerodynamics, sizing, routing-rules, duct-network, bypass, gost-segmentation, specification, dxf —
...
интеграция в `pipeline.test.ts` (кухня 90 → расход → Ø100 → ΔP); **Этап 9 — утка v2** (обвод по сетям,
несколько пересечений одной вытяжки, вертикальный зазор, автовыбор стороны, единый источник ξ отводов);
**Этап 10 — спецификация v2 и DXF v2** (масса звеньев в строках, привязка расходов/подбора Этапа 8,
группировка фиттингов по типу/размеру; DXF: фиттинги по типу — отвод-дуга, утка-S-полилиния, тройник/переход
линиями, размеры мм текстом, гильзы/клапаны на проходах через стены, легенда систем);
**Этап 11 — верификация норм**: шаг креплений сверен с полным текстом СП 73.13330.2016 п. 6.5.5
(размер <400 → 4 м, ≥400 → 3 м, `MOUNTING_SPACING_VERIFIED=true`), ξ отводов сосланы на справочник
проектировщика ОВиК (Староверов), табл. Л.3 задокументирована как допущение;
**Этап 12 — сквозная регрессия**: интеграционный тест v2 в `pipeline.test.ts` (комната → авто-зона →
расход 90 → П/В трассы → утка → сетевой подбор → спецификация v2 → DXF v2) + автоматизированная
UI-проверка через Playwright (панель «Вентиляция», «Расчёт сети», спецификация, CSV/DXF, маркировка П1/В1).
Аудит несоответствий ТЗ (2026-08-14) закрыл: высота прокладки — под потолком по умолчанию
(`DUCT_CLEARANCE_MM`, ceiling-режим включён; `C` — на пол для risers); П/В-пересечения больше НЕ
склеиваются авто-фиттингом (tee/cross только для трасс одной системы; клавиша **S** — П/В/Р при
рисовании); отводы получили радиус R/D (`radiusFactor`, R=1D/1.5D в ξ/спецификации/DXF); терминалы
норм-mount по типу (diffuser→ceiling, return-grille→wall); вертикальные крепления — шаг ≤4,5 м
(СП 73 п.6.5.7); седелка/зонт добавлены в номенклатуру (`fittingType` saddle/hood).
итог `bun test` — **3236 pass / 1 skip / 0 fail**).
**Агент автотрассировки** (план `docs/mep/PLAN-AGENT.md`: эскиз П/В полилиниями → кнопка
«Трассировка» → сеть по нормам; контекстное меню типа помещения; высоты «как в Revit»;
рекомендации оборудования; баланс П/В; митигации W1–W13) — **Этапы A1–A3 выполнены**:
A1 — `DuctSketchNode` (`packages/core/src/schema/nodes/duct-sketch.ts`: `runs`
`{system, points[{x, z, elev: 'auto' | {axisM}}]}`, один узел на уровень; plan-only kind
с пунктирным рендером на плане, `bake: 'strip'`, исключён из спецификации/DXF/GLB — тестами);
A2 — режим «Эскиз» в duct-инструменте (`duct-segment/tool.tsx`; **K** — переключение
Сборка↔Эскиз, клики копят полилинию, **Enter**/двойной клик завершает рану в узел уровня
одним undo-шагом, **S** — цикл П/В/Р; чистая логика наброска в
`packages/nodes/src/duct-sketch/sketch-draft.ts`);
A3 — контекстное меню «Тип помещения»: правый клик по зоне на 2D-плане выбирает её и
открывает меню у курсора с подменю категорий СП 54 (`SPACE_CATEGORIES`, радио-пункты
с нормативным расходом); выбор патчит `spaceCategory` одним undo-шагом (авто- и ручные
...
`mep/elevation-marks.ts`; **Этап D — сквозная регрессия**: интеграционный тест агента
end-to-end в `pipeline.test.ts` (комнаты → тип помещения → терминалы + установка → эскиз
П/В → «Трассировка» → сеть СП 60/фиттинги/утка/высоты → спецификация + DXF) + Playwright
UI-проверка связки агента в живом редакторе; добивка пробелов аудита: «Очистить эскиз»
(`planSketchClear` + кнопка), UI «Предпочтения трассировки» (передаются в buildDuctPlan),
ризеры (пары отводов 90° + вертикальное звено в materialize), предупреждение параллельных
П/В в узком коридоре (`parallel-pv-clearance`), W5-предупреждение перед rebuild,
статус `FITTING_ZETA_VERIFIED` в UI; 3351 тест
зелёные). Агент-сценарий PLAN-AGENT (A1–A3, B1–B5, C1–C4, D) — полностью реализован.
