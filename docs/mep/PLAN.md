...
**Следующий блок — Этап 13 (публикация).** Реализация по той же схеме: каждый этап завершается тестами, перед переходом — `bun check` + `bun run check-types` + `bun test` зелёные.
