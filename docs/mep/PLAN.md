...
**Все этапы 0–10 выполнены.** Целевой репозиторий: `pascalorg/editor` (монорепо Turborepo + Bun). Ветка разработки: `main`.
**Следующий блок — Этапы 11–13 (углубление): верификация норм, сквозная регрессия, публикация.** Реализация по той же схеме: каждый этап завершается тестами, перед переходом — `bun check` + `bun run check-types` + `bun test` зелёные.
