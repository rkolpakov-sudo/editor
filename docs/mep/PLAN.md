...
Статус агента: **этапы A1–A3, B1–B5, C1–C4 и D выполнены** (эскизный узел, режим «Эскиз»,
контекстное меню «Тип помещения»; ядро агента в `packages/core/src/mep/agent/*` от валидации
до плана построения; материализация «Трассировкой» в панели «Вентиляция», рекомендации
оборудования, автоответвления; C4 — отметки оси/низа в DXF и спецификации по ГОСТ 21.602;
D — интеграционный тест агента end-to-end + Playwright UI-проверка; добивка пробелов:
«Очистить эскиз», UI предпочтений, ризеры, параллель П/В, W5/W8 статусы; 3351 тест зелёные).
