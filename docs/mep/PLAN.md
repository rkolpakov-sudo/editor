...
**Все этапы 0–9 выполнены.** Целевой репозиторий: `pascalorg/editor` (монорепо Turborepo + Bun). Ветка разработки: `main`.
**Следующий блок — Этапы 10–13 (углубление): спецификация и DXF v2, верификация норм, сквозная регрессия, публикация.** Реализация по той же схеме: каждый этап завершается тестами, перед переходом — `bun check` + `bun run check-types` + `bun test` зелёные.
